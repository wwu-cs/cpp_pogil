...
int main() {
  ...
  \Q Circle the {\tt if/else} statement that is {\it nested} inside of 
    ...
    \begin{answer}[0.4in]
      The {\tt if/else} statement from lines 12-16 is nested inside the one from lines 9-17.
    \end{answer}
  ...
  \Q This code can be found in the file {\tt activity06c.cpp}.  Give three different grade
    ...
    \begin{center}
      ...
      \begin{tabular}{|c|c|p{4in}|}
        ...
        \hline
        1 & \ans[0.5in] {95} & \ans[2in] {Tests lines 9-10} \\
        \hline
        2 & \ans[0.5in] {82} & \ans[2in] {Tests lines 11-13} \\
        \hline
        3 & \ans[0.5in] {55} & \ans[2in] {Tests lines 11, 12, 14-15} \\
        \hline
      \end{tabular}
    \end{center}
  ...
  if (grade >= 90) {
    ...
    \begin{enumerate}
      \item Replace lines 9-17 in model 3 with this code. How does the output change?
        \begin{answer}[0.5in]
...
if (grade >= 90) {
  ...
  cout << "Poor." << endl;
      ...
      \item Does it make a difference where you add the additional {\tt else/if} statement? \key\\[-2.5mm]
        Compare adding it at line 3 in the code snippet above vs. at line 5.
        \begin{answer}[1in]
          Yes, it makes a difference. If we add it at line 5 instead of at line 3, then the ``Good!'' option is never used because
      ...
      \item Is the use of the final {\tt else} statement mandatory when creating an {\tt if/else/if} statement?  
        ...
        \begin{answer}[0.75in]
          Not mandatory.  Dropping the else statement will just print nothing for grades less than 60.
        \end{answer}
