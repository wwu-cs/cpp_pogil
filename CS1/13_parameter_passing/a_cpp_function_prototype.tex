...
Test Three: 3
  ...
  \Q A prototype for the function {\tt sum} is defined on line 1
    ...
    \begin{enumerate}
      ...
      \item How many arguments does the call to this function on line 4 have? \hfill
        \ans[1.5in]{four}

      \item How many arguments does the call to this function on line 5 have? \hfill
        \ans[1.5in]{three}

      \item How many arguments does the call to this function on line 6 have? \hfill
        \ans[1.5in]{two}
    \end{enumerate}
  ...
  \Q What are the values of the parameters {\tt a}, {\tt b}, {\tt c}, and {\tt d}
    ...
    \begin{enumerate}
      \itemsep 10pt
      \item Line 4: \cpp{sum(1,2,3,4)} \hfill 
          {\tt a =} \ans[0.5in]{1} \hspace{5pt} 
          {\tt b =} \ans[0.5in]{2} \hspace{5pt}
          {\tt c =} \ans[0.5in]{3} \hspace{5pt} and
          {\tt d =} \ans[0.5in]{4}
      \item Line 5: \cpp{sum(1,2,3)} \hfill
          {\tt a =} \ans[0.5in]{1} \hspace{5pt} 
          {\tt b =} \ans[0.5in]{2} \hspace{5pt}
          {\tt c =} \ans[0.5in]{3} \hspace{5pt} and
          {\tt d =} \ans[0.5in]{0}
      \item Line 6: \cpp{sum(1,2)} \hfill 
          {\tt a =} \ans[0.5in]{1} \hspace{5pt} 
          {\tt b =} \ans[0.5in]{2} \hspace{5pt}
  ...
  \Q Rewrite the prototype of the {\tt sum} function from line
    ...
    \begin{enumerate}
      ...
      \item \cpp{sum(3,2)} $\rightarrow$ 8 \hfill
        \ans[4.5in]{\cpp{int sum(int a, int b, int c=2, int d=1)}}

      \item \cpp{sum(1)} $\rightarrow$ 10 \hfill
        \ans[4.5in]{\cpp{int sum(int a, int b=1, int c=6, int d=2)}}
    \end{enumerate}
    
  \Q The file {\tt activity13b.cpp} contains the code from the
    model as well as the function definition.  Replace the function
    prototype on line 1 of that file with each of the following. Place a check 
    ...
    \begin{enumerate}
      ...
      \itemsep 5pt
        ...
        \item \ans[0.25in]{\checkmark} {\cpp{int sum(int a, int b, int c=0, int d=0)}}
        \item \ans[0.25in]{} {\cpp{int sum(int a=0, int b=0, int c, int d)}}
        \item \ans[0.25in]{\checkmark} {\cpp{int sum(int a=0, int b=0, int c=0, int d=0)}}
    \end{enumerate}
  ...
  \Q Follow the steps below to write a function that returns the product of
    ...
    Use the file {\tt activity13b.cpp} to test your code.
    \begin{itemize}
      \small
      \begin{multicols}{3}
