...
double total   = subCost + subTax
  ...
  \Q Determine which code block above ($X$ or $Y$) matches the
    ...
    \begin{enumerate}
      ...
      \item Uses more variables: \hfill \ans[1in]{$Y$}
      \item Would be easier to edit or debug: \hfill \ans[1in]{$Y$}
    \end{enumerate}
  ...
  \Q Each pair of code blocks below is syntactically and
    ...
    \begin{tabular}{|c|p{2.8in}|p{2.8in}|c|}
      \hline
...
float total = cost + tax + ship;
  ...
  \Q Rewrite the code below to be self-documenting.  Use
    your style recommendations\key\\[-2.5mm] above.
    \begin{center}
      \begin{minipage}{6in}
        \begin{cpplst}
