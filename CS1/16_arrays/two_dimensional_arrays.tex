\model{Two Dimensional Arrays} \\[-20pt]
  \begin{center}
    \begin{minipage}{4in}
      \begin{cpplst}[
        frame=lines,
    ...
    \begin{center}
      ...
      \begin{tabular}{|p{1.75in}|p{1.75in}|p{1.75in}|}
        \hline
          ...
          \tt \ans[1.25in]{\cpp{tableTwo[0][1]}} = 'B' &
          \tt \ans[1.25in]{\cpp{tableTwo[0][2]}} = 'C' \\
        \hline
          \tt \ans[1.25in]{\cpp{tableTwo[1][0]}} = 'D' &
          \tt \ans[1.25in]{\cpp{tableTwo[1][1]}} = 'E' &
          \tt \ans[1.25in]{\cpp{tableTwo[1][2]}} = 'F' \\
        \hline
          \tt \ans[1.25in]{\cpp{tableTwo[2][0]}} = 'G' &
          \tt \ans[1.25in]{\cpp{tableTwo[2][1]}} = 'H' &
          \tt \ans[1.25in]{\cpp{tableTwo[2][2]}} = 'I' \\
        \hline
      \end{tabular}
    \end{center}
...
for (int i = 0; i < 200; i++) {
  cout << tableTwo[i][0] << " ";
      ...
      \item If there were \cpp{int row} elements per column, write code to print out the
        \cpp{int k}th column.\hspace{5pt}\key\\[-2.5mm]
        \begin{answer}[1in]
          \scriptsize\vskip -25pt\null\hspace{1in}
          \begin{minipage}{2.5in}
            \begin{cpplst}[
