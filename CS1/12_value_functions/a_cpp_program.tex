\model{A C++ Program}
  \begin{tabular}{c c}
    \begin{center}
      \begin{minipage}{3in}
        \scriptsize
        \begin{cpplst}
#include <iostream>

using namespace std;

int getSmaller(int num1, int num2) {
  int smaller;
  if (num1 < num2) {
    smaller = num1;
  } else {
    smaller = num2;
  }
  return smaller;
}

int main() {
  int userNum1, userNum2;
  cout << "Enter two numbers separated by a space: ";
  cin >> userNum1 >> userNum2;
  int smallerNum = getSmaller(userNum1, userNum2);
  cout << "The smaller number is " << smallerNum << endl;
}
        \end{cpplst}
      \end{minipage}
    \end{center}
    &
    \begin{minipage}[b]{3.5in}
      {\bf Output:} 
      \hrule\vskip 5pt
      Enter two numbers separated by a space: 52 36\\
      The smaller number is 36
    \end{minipage}
  \end{tabular}
  
  {\it\large Refer to Model 1 above as your team develops consensus answers
    to the questions below.}

  \quest{10 min}
  
  \Q What does the function defined on lines 5-13 do? The code is found in {\tt activity12a.cpp}.
    \begin{answer}[0.25in]
      It returns the smaller of the two integer arguments passed to it.
    \end{answer}
    
  \Q So far you have created `void` functions which do not send any
    information back to the calling code. A {\it value-returning} function
    does send information of a particular type back to the calling code.
    \begin{enumerate}
      \itemsep 5pt
      \item What type of information does the function {\tt getSmaller} return? \hfill
        \ans[1.8in]{An integer}

      \item On what line of code is the function call to {\tt getSmaller}? \hfill
        \ans[1.8in]{Line 19}

      \item On what line does the function {\tt getSmaller} send back a value? \hfill
        \ans[1.8in]{Line 12}
    \end{enumerate}    

  \vskip -10pt

  \Q In a {\it void function} the {\it function call} is on a line by itself.
    Why is the function call in this model on the right-hand-side of an
    assignment statement?
    \begin{answer}[0.3in]
      Because it returns a value that is being assigned to the variable {\tt
      smallerNum}.
    \end{answer}      

  \vskip -15pt
    
  \Q If you wanted to alter this function to select and return the
    smaller of two {\tt  double}\key\\[-2.5mm] values, what changes would you
    need to make?
    \begin{answer}[0.4in]
      \fs
      You need to change the \cpp{int} type to a
      \cpp{double} in the following places.
      \begin{itemize}
        \begin{multicols}{2}
          \item In the three instances on line 5
          \item In the one instance on line 6
        \end{multicols}
      \end{itemize}
    \end{answer}