\model{Modeling with Graphs}
  ...
  \Q What real life scenario (not from above list) could you model with a directed graph?
    \begin{answer}[0.5in]
      The flow of traffic through a city could be modeled with a directed graph.
    \end{answer}

  \vskip -10pt

  \Q What real life scenario (not from above list) could you model with an undirected\key\\[-2.5mm] graph?
    \begin{answer}[0.5in]
      A friendship network among people could be modeled with an undirected graph.
    \end{answer}
